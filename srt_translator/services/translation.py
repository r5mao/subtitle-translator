...
class TranslationService:
    ...
    async def translate_texts(
        ...
        cleaned_texts = [
            self._preprocess_subtitle_text(t) if t.strip() else t for t in texts
        ]
        ...
        results = await translator.translate(
            to_translate, dest=target_lang, src=source_lang
        )
        if isinstance(results, list):
            translated = [
                self._postprocess_subtitle_text(r.text) if hasattr(r, "text") else ""
                for r in results
            ]
        else:
    ...
    async def translate_subtitle_entries_async(
        ...
        logger.info(
            f"Starting batch translation of {total_entries} entries from {source_lang} to {target_lang}"
        )
        async with Translator() as translator:
            ...
            for i in range(0, len(all_lines), batch_size):
                ...
                translated_batch = await self.translate_texts(
                    batch, source_lang, target_lang, translator
                )
            ...
            for entry, line_count in zip(entries, entry_line_counts):
                ...
                if (entry.sequence_number % 10 == 0) or (entry == entries[-1]):
                    logger.info(
                        f"Translated entry {entry.sequence_number}/{total_entries}"
                    )
